...
function MultiEditBar({ selectedGoals, updateGoal }) {
    ...
    const updateGoalsHierarchies = hierarchy => {
        selectedGoals.forEach(goal => {
            updateGoal({
                content: goal.content,
                user_hash: goal.user_hash,
    ...
    return (
        <>
                { viewsOpen.hierarchy && <HierarchyPicker selectedHierarchy={selectedGoals[0].hierarchy} hierarchyClicked={updateGoalsHierarchies}/> }
        ...
        <div className='multi_edit_bar'>
            ...
            <div className='multi_edit_bar_item' onClick={() => setViews({ ...defaultViews, hierarchy: !viewsOpen.hierarchy })}>
