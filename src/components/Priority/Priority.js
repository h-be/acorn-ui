...
function Priority({
  ...
  useEffect(() => {
    ...
    let goal_vote = {
      ...
      urgency: parseInt(value["Urgency"]) / 100 || 0,
      importance: parseInt(value["Importance"]) / 100 || 0,
      impact: value["Impact"] / 100,
      effort: parseInt(value["Effort"]) / 100 || 0,
      agent_address: whoami.entry.address,
      unix_timestamp: Date.now()
    };
    fetchGoalVotes().then(value => {
      const votes = value.filter(v => {
        return v.entry.goal_address === goalAddress;
      });
      const vote = votes.find(v => {
        return v.entry.agent_address === whoami.entry.address;
      });
      console.log("valu", votes.length, vote);
      if (votes.length > 0 && vote !== undefined) {
        console.log("update", goal_vote);
...
/*const mapDispatchToProps=(dispatch)=> {
  return {
    createGoalVote: (Vote) => {
      return dispatch(addVoteOfGoal.create(Vote))
    }
}
