...
function checkConflict(coordinates, withHierarchies) {
    ...
    if (childs)
      childs.forEach(value => {
        moveFamily(
          value,
          delta,
          withHierarchies.find(value2 => {
            return value2.goal.address === value;
          }).childs
        );
      });
  };
  const childsConflict = childs => {
    if (childs) {
      ...
      childs.forEach(address => {
        Object.keys(coordinates).forEach(value => {
          if (
            value !== address &&
            ((coordinates[value].x === coordinates[address].x &&
              coordinates[value].y === coordinates[address].y) ||
              (coordinates[value].x < coordinates[address].x &&
                coordinates[address].x < coordinates[value].x + totalWidth &&
                coordinates[value].y === coordinates[address].y))
          ) {
            bool = { bool: true, value: 1 };
          } else if (
            coordinates[address].x < coordinates[value].x &&
            coordinates[value].x < coordinates[address].x + totalWidth &&
            coordinates[value].y === coordinates[address].y
          ) {
            bool = { bool: true, value: -1 };
          }
...
function mapHierarchyToPosition(
  ...
  const indexInTierBrother = sameTierBrother
    .map(wH => wH.goal.address)
    .indexOf(goal.address);
  ...
  if (hierarchy === 1) {
    ...
    x =
      coordinates[parent].x +
      indexInTierBrother * totalWidth -
      ((sameTierBrother.length - 1) * totalWidth) / 2;
  }
