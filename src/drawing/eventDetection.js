...
export function checkForGoalAtCoordinatesInBox( width, goals, edges, convertedClick,convertedIni) {
    ...
    Object.keys(goals).map(address => goals[address]).forEach(goal => {
        ...
        // if click occurred within the box of a Goal
        if (convertedIni.x<coords.x && bottomRight.x<convertedClick.x &&
            convertedIni.y<coords.y && bottomRight.y<convertedClick.y ||
            convertedIni.x>bottomRight.x && coords.x > convertedClick.x &&
            convertedIni.y>bottomRight.y && coords.y > convertedClick.y||
            convertedIni.x>bottomRight.x && coords.x > convertedClick.x &&
            convertedIni.y<coords.y && bottomRight.y<convertedClick.y ||
            convertedIni.x<coords.x && bottomRight.x<convertedClick.x &&
            convertedIni.y>bottomRight.y && coords.y > convertedClick.y
